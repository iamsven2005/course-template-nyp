...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        try:
            file_extension = file.filename.rsplit('.', 1)[1].lower()
            # Extract images from the uploaded DOCX or XLSX file
            images_base64 = extract_images_as_base64(file, file_extension)

            # Base64 image to be added at the beginning
            base64_img_first = images_base64[0][1] if images_base64 else None
            if base64_img_first:
                doc_io = create_document(images_base64, base64_img_first, file)
                return send_file(doc_io, as_attachment=True, download_name='output.docx')
            else:
                return "No images found in the document", 400
